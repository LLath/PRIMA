namespace L05_Reflection {
  ...
  function boundaryDetection(_node: f.Node): void {
    ...
    const nodeScaling: f.Vector3 = nodeLocal.scaling;
    const xPosition: number = nodeLocal.translation.x;
    const yPosition: number = nodeLocal.translation.y;
    const xBoundary: number = (boundary.x / CAMERA_POSITION) * nodeScaling.x;
    const yBoundary: number =
      (boundary.y / CAMERA_POSITION) * nodeScaling.y - nodeScaling.y;
